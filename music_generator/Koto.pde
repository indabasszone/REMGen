...
class Koto implements Instrument {
    ...
    for (int i = 0; i < rememberMelody1.length; i++) {
      if (!stopped) {
        if (rememberRandom1[i] % playProb == 0){
          rememberMelody1[i].play(1, (i % 4) + (1 - (1.15 * (i % 4))));
        }
